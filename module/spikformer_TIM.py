...
class TIM(nn.Module):
    def __init__(self, args, in_channels=16, TIM_alpha=0.5):
        super().__init__()
        ...
        self.in_lif = LIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan(alpha=5.0), backend='cupy', step_mode='m')
        self.out_lif = LIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan(alpha=5.0), backend='cupy', step_mode='m')
...
class SSA(nn.Module):
    def __init__(self, args, dim,  num_heads=16, TIM_alpha=0.5):
        super().__init__()
        ...
        if args.use_tim:
            self.TIM = TIM(args, TIM_alpha=TIM_alpha, in_channels=self.in_channels)
...
class Block(nn.Module):
    def __init__(self, args, dim, num_heads, step=10, TIM_alpha=0.5, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.attn = SSA(args, dim,  TIM_alpha=TIM_alpha, num_heads=num_heads)
...
class Spikformer(nn.Module):
    def __init__(self,args, step=10, TIM_alpha=0.5, if_UCF=False,
        ...
        block = nn.ModuleList([Block(args, step=step, TIM_alpha=TIM_alpha,
...
if __name__ == '__main__':
    ...
    # 初始化模型
    model = Spikformer(args).to(device)
