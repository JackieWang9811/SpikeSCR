from datasets import SHD_dataloaders_teacher_student, SSC_dataloaders, GSC_dataloaders
from configs.best_config_SHD_former import Config as SHDTConfig
from configs.best_config_SHD_former_teacher import  Config as TSHDTConfig
...
def calc_loss(config, output, y, loss_weight=1.0):
    if config.loss == 'mean':
    ...
    if config.loss_fn == 'CEloss':
        ...
        loss = CEloss(m, y)
        weighted_loss = torch.mean(loss * loss_weight)  # 使用权重调整损失
        return weighted_loss

def get_loss_weight(epoch, num_epochs):
    # 三个阶段的分界点
    long_seq_end = 0.1 * num_epochs
    medium_seq_end = 0.6 * num_epochs

    if epoch <= long_seq_end:
        return 1.0  # 长序列阶段
    elif epoch <= medium_seq_end:
        return 0.5  # 中序列阶段
    else:
        return 0.2  # 短序列阶段
...
def train_model(config, train_loader, teacher_train_loader,  valid_loader, test_loader, device, model, optimizer, schedulers, num_epochs):
...
if __name__ == '__main__':
    ...
    config = SHDTConfig()
    teacher_config = TSHDTConfig()
    config.log_dir = './logs/logging/shd/PTCD'
    ...
    if config.dataset == 'shd':
        teacher_train_loader, train_loader, valid_loader = SHD_dataloaders_teacher_student(config, teacher_config)
        test_loader = None
    ...
    for hidden_dim in config.n_hidden_neurons_list:
        ...
        teacher_config.hidden_dims = config.mlp_ratio * hidden_dim
        teacher_model = SpikeDrivenTransformer(teacher_config).to(device)

        ## 最好的teacher，不需要在训练的
        model_weights_path = '/data/wjq/SNN-delays/checkpoints/shd/spike-temporal-former||seed=312||shd||1depths||10ms||bins=5||lr_w=0.01||heads=8_Best_Loss_2024-07-27_16-00-54.pt'  # depth=1
        # model_weights_path = '/data/wjq/SNN-delays/checkpoints/ssc/spike-temporal-former||seed=312||shd||1depths||2ms||bins=5||lr_w=0.005||heads=8_Best_Loss_2024-07-10_09-59-03.pt'  # depth=1
        ...
        total_iters = len(train_loader) * max(1, (config.epochs - config.n_warmup))
        ...
        train_model(config, train_loader,teacher_train_loader, valid_loader, test_loader, device, model, optimizer, schedulers, epochs)